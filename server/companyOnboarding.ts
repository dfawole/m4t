...
async function sendApplicationConfirmationEmail(
  ...
  const msg = {
    ...
    subject: `Enterprise Application Received - ${companyName}`,
    html: `
...
async function sendInternalNotificationEmail(application: any): Promise<boolean> {
  const msg = {
    to: 'enterprise@m4t.com', // Internal enterprise team email
    from: 'noreply@m4t.com',
    subject: `New Enterprise Application - ${application.companyName}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
async function sendStatusUpdateEmail(application: any, status: string): Promise<boolean> {
  ...
  const msg = {
    ...
    from: 'noreply@m4t.com',
    subject: `${statusInfo.subject} - ${application.companyName}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
export function registerCompanyOnboardingRoutes(app: any) {
  ...
  app.patch("/api/company-onboarding/applications/:applicationId/status", updateCompanyOnboardingApplicationStatus);
}