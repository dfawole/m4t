...
export async function getUserBadges(req: Request, res: Response) {
	// req.user is now typed: { id?: string } & JwtPayload
	const user = req.user;

	// prefer id if your User object has one, otherwise fallback to JWT sub
...
export async function updateChallengeProgress(req: Request, res: Response) {
	try {
		...
		const userChallenge = await storage.updateUserChallengeProgress(userId, challengeId, progress);
	...
	try {
		...
		for (const challenge of activeChallenges) {
			...
			if (criteria && criteria.type === 'lesson_completion') {
				...
				await storage.updateUserChallengeProgress(userId, challenge.challengeId, newProgress);
