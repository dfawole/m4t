...
(async () => {
  ...
  const port = parseInt(process.env.PORT || (process.env.NODE_ENV === 'production' ? '3006' : '5000'));
  const host = '0.0.0.0'; // Required for Replit deployment
  
  server.listen(port, host, () => {
    log(`M4T Learning Platform serving on ${host}:${port} (${process.env.NODE_ENV || 'development'})`);
  });
})();
