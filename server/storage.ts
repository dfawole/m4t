...
export interface IStorage {
  ...
  updateCompanyStripeInfo(companyId: number, stripeInfo: { customerId: string, subscriptionId: string }): Promise<Company>;
  
  // Subscription plan operations
  createSubscriptionPlan(plan: any): Promise<SubscriptionPlan>;
  getSubscriptionPlans(): Promise<SubscriptionPlan[]>;
  getSubscriptionPlan(id: number): Promise<SubscriptionPlan | undefined>;
  updateSubscriptionPlanStripeInfo(planId: number, stripeInfo: { productId: string, priceId: string }): Promise<SubscriptionPlan>;
  ...
  getUserCourseProgress(userId: string, courseId: number): Promise<{ completed: number, total: number }>;
  
  // Emoji Reaction operations
  addEmojiReaction(data: InsertEmojiReaction): Promise<EmojiReaction>;
  removeEmojiReaction(userId: string, contentType: string, contentId: number, reactionType: string): Promise<void>;
  getContentEmojiReactions(contentType: string, contentId: number): Promise<{ type: string, count: number }[]>;
  getUserReactionToContent(userId: string, contentType: string, contentId: number): Promise<string | null>;
  
  // Company onboarding operations
  createCompanyOnboardingApplication(data: InsertCompanyOnboardingApplication): Promise<CompanyOnboardingApplication>;
  ...
  updateCompanyOnboardingApplicationStatus(id: number, status: string, reviewedBy: string, reviewNotes?: string): Promise<CompanyOnboardingApplication>;
  ...
  addPointsToUser(userId: string, points: number, description: string, transactionType: string, entityType?: string, entityId?: string): Promise<{ userPoints: UserPoints, transaction: PointTransaction }>;
  getUserPointTransactions(userId: string, limit?: number): Promise<PointTransaction[]>;
  getUserLeaderboard(limit?: number): Promise<{ userId: string, username: string, totalPoints: number, level: number }[]>;
  ...
  updateUserChallengeProgress(userId: string, challengeId: number, progress: number): Promise<UserChallenge>;
...
export class DatabaseStorage implements IStorage {
  ...
  async addEmojiReaction(data: InsertEmojiReaction): Promise<EmojiReaction> {
    ...
    const [reaction] = await db.insert(emojiReactions)
      .values(data)
      .returning();
  ...
  async removeEmojiReaction(userId: string, contentType: string, contentId: number, reactionType: string): Promise<void> {
    await db.delete(emojiReactions)
      .where(and(
  ...
  async getContentEmojiReactions(contentType: string, contentId: number): Promise<{ type: string, count: number }[]> {
  ...
  async getUserReactionToContent(userId: string, contentType: string, contentId: number): Promise<string | null> {
  ...
  // User Points operations
  async createOrUpdateUserPoints(userId: string, data: Partial<InsertUserPoints>): Promise<UserPoints> {
  ...
  async getUserLeaderboard(limit: number = 10): Promise<{ userId: string, username: string, totalPoints: number, level: number }[]> {
  ...
  async updateUserStreak(userId: string, increment: boolean): Promise<Streak> {
    ...
    const lastActivity = currentStreak.lastActivityDate ? new Date(currentStreak.lastActivityDate) : null;
  ...
  async updateUserChallengeProgress(userId: string, challengeId: number, progress: number): Promise<UserChallenge> {
    const [userChallenge] = await db
  ...
  async getUserActiveChallenges(userId: string): Promise<(UserChallenge & { challenge: Challenge })[]> {
    return db
      .select({
  ...
  async getUserCompletedChallenges(userId: string): Promise<(UserChallenge & { challenge: Challenge })[]> {
    return db
      .select({
        userChallenge: userChallenges,
        challenge: challenges,
  ...
  async updateCompanyStripeInfo(companyId: number, stripeInfo: { customerId: string, subscriptionId: string }): Promise<Company> {
  ...
  async updateSubscriptionPlanStripeInfo(planId: number, stripeInfo: { productId: string, priceId: string }): Promise<SubscriptionPlan> {
    const [plan] = await db
  ...
  async getCompanyActiveSubscription(companyId: number): Promise<CompanySubscription | undefined> {
    const [subscription] = await db
      ...
      .where(and(
        ...
        eq(companySubscriptions.isActive, true)
      ))
      .orderBy(desc(companySubscriptions.createdAt))
      .limit(1);
    return subscription;
  ...
  async searchCourses(query: string): Promise<Course[]> {
    return await db
      ...
      .from(courses)
      .where(
        sql`to_tsvector('english', ${courses.title} || ' ' || ${courses.description}) @@ to_tsquery('english', ${query.replace(/ /g, " & ")})`
      );
  }
  ...
  async getUserCourseProgress(userId: string, courseId: number): Promise<{ completed: number, total: number }> {
    // Get all modules for the course
    const courseModules = await this.getModulesByCourse(courseId);
    const moduleIds = courseModules.map(module => module.id);
    ...
    const completedLessonsResult = lessonIds.length > 0 
      ? await db
          .select({ count: count() })
          .from(lessonCompletions)
          .where(and(
  ...
  async getContentReactions(contentType: string, contentId: number): Promise<{reactions: any[], userReaction: string | null}> {
    try {
  ...
  async getUserReaction(userId: string, contentType: string, contentId: number): Promise<string | null> {
    try {
  ...
  async addOrUpdateReaction(userId: string, contentType: string, contentId: number, reactionType: string): Promise<EmojiReaction> {
    try {
  ...
  async removeReaction(userId: string, contentType: string, contentId: number, reactionType: string): Promise<void> {
    try {
      await db
  ...
  // Company onboarding operations
  async createCompanyOnboardingApplication(data: InsertCompanyOnboardingApplication): Promise<CompanyOnboardingApplication> {
    const [application] = await db
      .insert(companyOnboardingApplications)
      .values(data)
      .returning();
    return application;
  }
  ...
  async getCompanyOnboardingApplication(id: number): Promise<CompanyOnboardingApplication | undefined> {
    const [application] = await db
      .select()
      .from(companyOnboardingApplications)
