...
const PLATFORM_EMAIL = 'noreply@learnflow.com';
...
export async function sendLicenseAssignmentEmail(email: string, companyName: string): Promise<boolean> {
  if (!SENDGRID_API_KEY) {
    console.warn('SendGrid API key not set, skipping email send');
    return false;
  }
...
export async function sendEnrollmentConfirmationEmail(email: string, courseName: string): Promise<boolean> {
  if (!SENDGRID_API_KEY) {
    console.warn('SendGrid API key not set, skipping email send');
    return false;
  }
