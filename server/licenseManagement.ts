...
import { sendLicenseAssignmentEmail, sendLicenseRevokedEmail, sendLicenseSuspendedEmail } from "./emailService";
...
export function isCompanyAdmin(req: Request, res: Response, next: Function) {
  if (req.user && (req.user.role === UserRole.COMPANY_ADMIN || req.user.role === UserRole.INTERNAL_ADMIN)) {
    return next();
  }
...
async function logLicenseActivity(licenseId: string, userId: string, activity: string, req: Request) {
  try {
...
export async function createLicensesForSubscription(req: Request, res: Response) {
  try {
    ...
    if (!companyId || !subscriptionId || !quantity || isNaN(quantity) || quantity <= 0) {
      return res.status(400).json({ message: "Invalid parameters. Required: companyId, subscriptionId, and quantity." });
    }
...
export function registerLicenseManagementRoutes(app: any) {
  ...
  app.get('/api/companies/:companyId/licenses/available', isCompanyAdmin, getAvailableCompanyLicenses);
  app.get('/api/companies/:companyId/licenses/assigned', isCompanyAdmin, getAssignedCompanyLicenses);
