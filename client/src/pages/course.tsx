...
export default function Course() {
  ...
  const { data: instructor, error: instructorError } = useQuery({
    queryKey: course?.instructorId ? [`/api/users/${course.instructorId}`] : null,
  ...
  // Handle enrollment in the course
  ...
  const handleStartCourse = () => {
    if (progress && course.modules && course.modules.length > 0) {
  ...
  const markLessonComplete = async (lessonId) => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.gamification && data.gamification.pointsEarned) {
          toast({
            title: "Points Earned!",
        ...
        if (data.gamification && data.gamification.badgesEarned && data.gamification.badgesEarned.length > 0) {
          toast({
        ...
        if (data.gamification && data.gamification.challengesCompleted && data.gamification.challengesCompleted.length > 0) {
          toast({
